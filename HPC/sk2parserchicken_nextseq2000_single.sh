...
cd /hpc/hub_kops/Sjoerd/rawdataandmapped/run33_KLA6273

gunzip *$A*_R1_001.fastq.gz

mkdir $A-output

/hpc/hub_kops/bin/bwa aln -q 0 -n 0.04 -k 2 -l 200 -t 4 -B 11 /hpc/hub_kops/GRCg7b.fa *$A*_R1_001.fastq > $A-output/$A-R1.sai

/hpc/hub_kops/bin/bwa samse /hpc/hub_kops/GRCg7b.fa $A-output/$A-R1.sai *$A*_R1_001.fastq > $A-output/$A-R1.sam
